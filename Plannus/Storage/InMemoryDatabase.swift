...
/// In-memory ``TaskDatabase`` for tests and previews. Reproduces the original
/// save semantics: `saveTasks` clears and re-inserts everything, assigning
/// sequential integer-derived string ids to tasks and subtasks.
public final class InMemoryDatabase: TaskDatabase {
    ...
    public func loadTasks() throws -> [Task] {
        // Mirror the DB's `ORDER BY createDate DESC`.
    ...
    public func saveTasks(_ newTasks: [Task]) throws {
        ...
        for var task in newTasks {
            guard !task.createDate.isEmpty else {
                throw PlannusError.validation("Task missing required field: createDate")
            }
    ...
    public func saveEvent(_ event: Event) throws {
        guard !event.title.isEmpty, !event.date.isEmpty else {
            throw PlannusError.validation("Event title and date are required")
        }
...
public enum PlannusError: Error, Equatable {
